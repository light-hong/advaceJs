...
(function (window) {
  ...
  */
  ...
  指定成功和失败的回调函数
  返回一个新的promise对象
  返回promise的结果又onResolved/onRejected执行结果决定
  */
  Promise.prototype.then = function (onResolved, onRejected) {
    ...
    return new Promise((resolve, reject) => {
      ...
      } else if (self.status === RESOLVED) { // 当前是resolved状态，异步执行onResolved并改变return的promise状态
        setTimeout(() => {
          handle(onResolved)
        })
      } else { // 当前是rejected状态，异步执行pnRejected并改变return的promise状态
        setTimeout(() => {
          handle(onRejected)
        })
      }
  ...
  */
  ...
  /* 
  自定义返回一个promise对象，它在指定的时间后才确定结果
  */
